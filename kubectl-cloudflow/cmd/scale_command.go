...
func (c *scaleApplicationCMD) scaleImpl(cmd *cobra.Command, args []string) {
	...
	if err != nil {
		printutil.LogAndExit("scale factor must be a number, %s", err.Error())
	}
	...
	if foundStreamlet == false {
		printutil.LogAndExit("Streamlet %s is not found in the application.", streamletName)
	}
	...
	if _, err := cloudflowApplicationClient.Update(*applicationCR); err != nil {
		printutil.LogAndExit("Failed to update the application %s, %s", applicationCR.ObjectMeta.Name, err.Error())
	}

	printutil.PrintSuccess("Streamlet %s in application %s is being scaled to %v replicas.\n", streamletName, applicationName, replicas)
}
