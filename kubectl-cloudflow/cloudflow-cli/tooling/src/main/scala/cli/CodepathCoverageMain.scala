...
object CodepathCoverageMain extends App {
  ...
  val flinkPvc = preparationClient.load(new FileInputStream("../cloudflow-it/src/it/resources/flink-pvc.yaml")).get()
  try {
  ...
  val sparkPvc = preparationClient.load(new FileInputStream("../cloudflow-it/src/it/resources/spark-pvc.yaml")).get()
  try {
  ...
  val cli = new Cli(None, (_, l) => Cli.defaultKubeClient(None, l))(logger) {
    @silent def transform[T](cmd: Command[T], res: T): T = res

    @silent def handleError[T](cmd: Command[T], ex: Throwable): Unit = ()
  }
