...
class CloudflowApplicationSpec
  ...
  "CloudflowApplication.CR" should {
    ...
    "report its status as Running when all streamlet pods are running and ready in a mixed app" in {
      ...
      status = status.updatePod("ingress", mkRunningReadyPod("ingress"))
      status.aggregatedStatus mustBe CloudflowApplication.Status.Pending
      (1 to SparkRunner.DefaultNrOfExecutorInstances + 1).foreach { _ =>
        status = status.updatePod("spark-egress", mkRunningReadyPod("spark-egress"))
        status.aggregatedStatus mustBe CloudflowApplication.Status.Pending
      }

      (1 to FlinkRunner.DefaultTaskManagerReplicas + 1).foreach { _ =>
        status = status.updatePod("flink-egress", mkRunningReadyPod("flink-egress"))
      }
      status.aggregatedStatus mustBe CloudflowApplication.Status.Running
    }
  ...
  def mkRunningReadyPod(streamletName: String) =
    mkPod(streamletName, Some(Pod.Phase.Running), containerStatuses = List(mkContainerStatus(ready = true)))

  def mkRunningNotReadyPod(streamletName: String) =
    mkPod(streamletName, Some(Pod.Phase.Running), containerStatuses = List(mkContainerStatus(ready = false)))
  def mkFailedPod(streamletName: String) =
    mkPod(streamletName, Some(Pod.Phase.Failed), containerStatuses = List(mkContainerStatus(ready = false)))
  def mkSucceededPod(streamletName: String) =
    mkPod(streamletName, Some(Pod.Phase.Succeeded), containerStatuses = List(mkContainerStatus(ready = false)))

  def mkWaitingPod(streamletName: String, reason: String) =
    mkPod(
      streamletName,
      Some(Pod.Phase.Pending),
      containerStatuses = List(mkContainerStatus(state = Some(Container.Waiting(Some(reason))), ready = false)))

  def mkTerminatingPod(streamletName: String) =
    mkPod(streamletName = streamletName, deletionTimestamp = Some(java.time.ZonedDateTime.now()))

  def mkTerminatedPod(streamletName: String) =
    mkPod(
      streamletName,
      Some(Pod.Phase.Pending),
      containerStatuses = List(mkContainerStatus(state = Some(Container.Terminated(0)), ready = false)))
