...
class SparkRunnerSpec
  ...
  "SparkRunner" should {
    ...
    "create a valid SparkApplication CR" in {
      ...
      val cr = sparkRunner.resource(deployment = deployment, app = app, configSecret = Secret(metadata = ObjectMeta()))
    ...
    "read from config custom secrets and mount them DIFFERENTLY to the driver and executor pods specs" in {
      ...
      cr.spec.driver.volumeMounts must contain allElementsOf List(Volume.Mount("foo", "/etc/my/file", true))

      cr.spec.executor.volumeMounts must contain allElementsOf List(Volume.Mount("bar", "/etc/mc/fly", false))
    ...
    "read from config custom pvc and mount them to the driver and executor pods specs" in {
      ...
      cr.spec.driver.volumeMounts must contain allElementsOf List(Volume.Mount("foo", "/etc/my/file", false))
    }
