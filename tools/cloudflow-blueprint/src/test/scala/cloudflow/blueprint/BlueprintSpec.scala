...
class BlueprintSpec extends AnyWordSpec with Matchers with EitherValues with OptionValues {
  ...
  "A blueprint" should {
    ...
    "be able to define and use streamlets" in {
      ...
      blueprint.streamlets(0) mustBe ingressRef.copy(verified = Some(VerifiedStreamlet(ingressRef.name, ingress)))
    }
    ...
    "be able to define, use and connect streamlets" in {
      ...
      blueprint.problems mustBe empty
      blueprint.streamlets(0) mustBe ingressRef.copy(verified = Some(VerifiedStreamlet(ingressRef.name, ingress)))
      blueprint.streamlets(1) mustBe processorRef.copy(verified = Some(VerifiedStreamlet(processorRef.name, processor)))
    }
    ...
    "not fail verification with UnconnectedInlets for already reported IncompatibleSchema problems" in {
      ...
      blueprint.problems.filter {
        ...
        case _                           => true
      } mustBe Vector(IncompatibleSchema(paths(0), paths(1)))
    }

    "fail verification for configuration parameters with invalid validation patterns" in {
      val blueprint =
        createBlueprintWithConfigurationParameter(
          ConfigParameterDescriptor("test-parameter", "", "string", Some("^.{1,65535$"), None))
    ...
    "fail verification for volume mounts with duplicate names" in {
      val blueprint = createBlueprintWithVolumeMounts(
        VolumeMountDescriptor("ml-data", separator + "some-path", "ReadWriteMany"),
    ...
    "fail verification for volume mounts with invalid names" in {
      val firstBlueprint =
        createBlueprintWithVolumeMounts(VolumeMountDescriptor("-ml-data", separator + "some-path", "ReadWriteMany"))
    ...
    "fail verification for volume mounts with invalid paths" in {
      val blueprint =
        createBlueprintWithVolumeMounts(VolumeMountDescriptor("ml-data", s"..${separator}some-path", "ReadWriteMany"))
    ...
    "fail verification for volume mounts with non-absolute paths" in {
      val blueprint = createBlueprintWithVolumeMounts(
        VolumeMountDescriptor("ml-data", s"some-path${separator}testing", "ReadWriteMany"))
    ...
    "be able to update streamlets" in {
      ...
      StreamletRef(
        ingressRef.name,
        "NewClassName",
        Vector(StreamletDescriptorNotFound(ingressRef.name, "NewClassName")),
        None)
    ...
    "be able to disconnect from streamlet with multiple inlets" in {
      ...
      blueprint.problems mustBe Vector(UnconnectedInlets(Vector(UnconnectedPort(mergeRef.name, merge.in1))))
    ...
    "be able to disconnect many streamlets" in {
      ...
      disconnected.problems mustBe Vector(
        UnconnectedOutlets(
          Vector(
            UnconnectedPort(ingressRef.name, ingress.outlets(0)),
            UnconnectedPort(processorRef.name, processor.outlets(0)))),
