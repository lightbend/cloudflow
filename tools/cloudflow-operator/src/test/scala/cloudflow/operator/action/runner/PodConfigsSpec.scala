...
class PodsConfigSpec extends WordSpecLike with OptionValues with TryValues with MustMatchers {
  ...
  "PodsConfig" should {
    "load from kubernetes config " in {
      ...
      podConfigs.pods("pod").containers("container").env(0) mustBe (EnvVar(
        "JAVA_OPTS",
        EnvVar.StringValue("-XX:MaxRAMPercentage=40.0")))
      ...
      podConfigs.pods("executor").containers("custom-container").env(0) mustBe (EnvVar(
        "MY_OPT_3",
        EnvVar.StringValue("3")))
    }
