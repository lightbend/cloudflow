...
class RunnerActionsSpec
  ...
  "RunnerActions" should {
    "create resources for runners when there is no previous application deployment" in {
      ...
      val currentApp = None
      val newApp = CloudflowApplication(
      ...
      val createDeploymentActions = actions.collect {
        case p: ProvidedAction[_] =>
          p.asInstanceOf[ProvidedAction[Secret]].getAction(Some(secret)).asInstanceOf[SingleResourceAction[Deployment]]
      }
    ...
    "update when the new applications requires the same runners as the current one" in {
      ...
      val updateDeploymentActions = actions.collect {
        case p: ProvidedAction[_] =>
          p.asInstanceOf[ProvidedAction[Secret]].getAction(Some(secret)).asInstanceOf[ResourceAction[Deployment]]
      }
    ...
    "delete runner resources when new app removes a runner" in {
      ...
      val currentApp = CloudflowApplication(
        CloudflowApplicationSpecBuilder.create(appId, appVersion, image, verifiedBlueprint, agentPaths))
  ...
  def assertAkkaDeployment(
    ...
    val streamletDeployment =
      app.deployments.find(streamletDeployment => Name.ofPod(streamletDeployment.name) == deployment.name).value
    ...
    if (streamletDeployment.endpoint.isEmpty) {
      ...
      deployment.spec
        .flatMap(_.strategy.map(_._type))
        .value mustEqual skuber.apps.v1.Deployment.StrategyType.RollingUpdate
    }
