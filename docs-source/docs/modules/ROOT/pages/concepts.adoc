...
==== FanOut
A FanOut has one inlet and more than one outlets. The FanOut splits the input into multiple data streams depending on some criteria. A typical example can be an implementation of validation where the FanOut splits the input into valid and invalid data streams.
...
The application gets formed according to the `Streamlets` included and the connections specified in the blueprint, materialized as data flows between the `Streamlets`.
