...
To install a new cluster, clone the https://github.com/lightbend/cloudflow/[repo] in your local system by selecting the latest release branch or download the latest release file from https://github.com/lightbend/cloudflow/releases[here], go to the `installer/` folder and follow the instructions bellow.
...
NOTE:: This step uses Helm which can be installed from https://v2.helm.sh/docs/using_helm/#installing-helm[here]. Currently only Helm 2 is supported while Helm 3 support is WIP.
Please make sure you have installed the right version of Helm on your local machine.
