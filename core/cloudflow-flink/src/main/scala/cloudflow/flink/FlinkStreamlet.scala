...
abstract class FlinkStreamletLogic(implicit val context: FlinkStreamletContext) extends StreamletLogic[FlinkStreamletContext] {
   ...
   *
   * @param inlet the inlet port to read from
   * @param dataconverter - data converter class allowing for custom error processing
   * @param clazz the class of data flowing from `inlet`
   * @return the data read as `DataStream[In]`
   */
  final def readStream[In](inlet: CodecInlet[In], dataconverter: InletDataConverter[In], clazz: Class[In]): JDataStream[In] =
    context.readStream(inlet, dataconverter)(TypeInformation.of[In](clazz)).javaStream
