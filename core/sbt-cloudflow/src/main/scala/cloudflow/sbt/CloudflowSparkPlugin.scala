...
object CloudflowSparkPlugin extends AutoPlugin {
      ...
      Seq(
        ...
        Instructions.Run.shell(
          Seq(
            ...
            Seq("cp", "-r", s"spark-${sparkVersion}-bin-cloudflow-${scalaVersion}/examples", s"${sparkHome}/examples"),
            Seq("cp", "-r", s"spark-${sparkVersion}-bin-cloudflow-${scalaVersion}/data", s"${sparkHome}/data"),
            Seq("cp", s"spark-${sparkVersion}-bin-cloudflow-${scalaVersion}/kubernetes/dockerfiles/spark/entrypoint.sh", "/opt/"),
            ...
            Seq("curl", "-L", s"https://github.com/krallin/tini/releases/download/${tiniVersion}/tini", "-o", "/sbin/tini"),
        ...
        Instructions.Expose(Seq(4040,5005))
      )
    },
  ...
  private lazy val metricsPropertiesContent =
    scala.io.Source.fromResource("runtimes/spark/metrics.properties", getClass().getClassLoader()).getLines.mkString("\n")
  ...
  private lazy val sparkEntrypointShContent =
    scala.io.Source.fromResource("runtimes/spark/spark-entrypoint.sh", getClass().getClassLoader()).getLines.mkString("\n")
