...
object CloudflowFlinkPlugin extends AutoPlugin {
  ...
  override def projectSettings =
    Seq(
      ...
      baseDockerInstructions := {
        ...
        val flinkTgzUrl = s"https://archive.apache.org/dist/flink/flink-${flinkVersion}/$flinkTgz"
        ...
        Seq(
          ...
          Instructions.Run.shell(
            Seq(
              ...
              Seq("chmod", "a+x", "/opt/flink-entrypoint.sh")).reduce(_ ++ Seq("&&") ++ _)),
  ...
  private lazy val flinkEntrypointContent =
    scala.io.Source
      .fromResource("runtimes/flink/flink-entrypoint.sh", getClass().getClassLoader())
      .getLines
      .mkString("\n")
