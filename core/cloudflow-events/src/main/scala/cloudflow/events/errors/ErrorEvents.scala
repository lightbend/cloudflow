...
import skuber.{ Event, ObjectEditor, ObjectMeta, ObjectReference, k8sInit }
...
object ErrorEvents {
  ...
  private[cloudflow] def newEvent(
      streamlet: StreamletDefinition,
      runnerType: String,
      throwable: Throwable,
      podName: String,
      podUid: String,
  ...
  private[cloudflow] def hash(value: String): String = {
    md5.digest(value.getBytes) // calling digest implicitly resets MD5 instance
      .take(5)
      .map("%02X".format(_))
      .mkString
      .toLowerCase
