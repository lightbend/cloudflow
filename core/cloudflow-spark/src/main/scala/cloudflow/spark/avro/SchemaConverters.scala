...
object SchemaConverters {
  ...
  def createConverterToSQL(sourceAvroSchema: Schema, targetSqlType: DataType): AnyRef ⇒ AnyRef = {
    ...
    def createConverter(avroSchema: Schema, sqlType: DataType, path: List[String]): AnyRef ⇒ AnyRef = {
      ...
      (sqlType, avroType) match {
        ...
        case (IntegerType, INT) | (BooleanType, BOOLEAN) | (DoubleType, DOUBLE) | (FloatType, FLOAT) | (LongType, LONG) ⇒
          identity
        ...
        case (sqlType, UNION) ⇒
          if (avroSchema.getTypes.asScala.exists(_.getType == NULL)) {
            ...
            avroSchema.getTypes.asScala.map(_.getType) match {
              case Seq(t1 @ _) ⇒ createConverter(avroSchema.getTypes.get(0), sqlType, path)
   ...
   */
  private def convertTypeToAvro[T](dataType: DataType, schemaBuilder: BaseTypeBuilder[T], structName: String, recordNamespace: String): T =
    dataType match {
  ...
  private def convertFieldTypeToAvro[T](dataType: DataType,
    ...
    dataType match {
      ...
      case ArrayType(elementType, _) ⇒
        val builder = getSchemaBuilder(dataType.asInstanceOf[ArrayType].containsNull)
        val elementSchema =
          convertTypeToAvro(elementType, builder, structName, getNewRecordNamespace(elementType, recordNamespace, structName))
        newFieldBuilder.array().items(elementSchema)

      case MapType(StringType, valueType, _) ⇒
        val builder     = getSchemaBuilder(dataType.asInstanceOf[MapType].valueContainsNull)
        val valueSchema = convertTypeToAvro(valueType, builder, structName, getNewRecordNamespace(valueType, recordNamespace, structName))
        newFieldBuilder.map().values(valueSchema)

      case structType: StructType ⇒
        convertStructToAvro(structType,
   ...
   */
  private[avro] def getNewRecordNamespace(elementDataType: DataType, currentRecordNamespace: String, elementName: String): String =
