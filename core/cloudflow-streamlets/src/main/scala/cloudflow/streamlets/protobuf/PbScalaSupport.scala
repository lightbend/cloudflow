...
object PbScalaSupport {
  val Format = "protobuf"

  def createSchemaDefinition(descriptor: Descriptor) = SchemaDefinition(
    name = descriptor.fullName,
    schema = schemaString(descriptor),
    fingerprint = fingerprintSha256(descriptor),
    format = Format
  )

  def schemaString(descriptor: Descriptor): String = descriptor.asProto.toProtoString

  private def fingerprintSha256(descriptor: Descriptor): String =
    Base64.getEncoder.encodeToString(MessageDigest.getInstance("SHA-256").digest(descriptor.asProto.toProtoString.getBytes("UTF-8")))
}
