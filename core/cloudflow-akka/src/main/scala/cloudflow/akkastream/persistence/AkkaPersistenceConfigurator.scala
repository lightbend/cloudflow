...
object AkkaPersistenceConfigurator {
  ...
  private final val CASSANDRACONTACTPOINT     = "datastax-java-driver.basic.contact-points"
  private final val CASSANDRAKEYSPACEJOURNAL  = "akka.persistence.cassandra.journal.keyspace"
  private final val CASSANDRAKEYSPACESNAPSHOT = "akka.persistence.cassandra.snapshot.keyspace"
  private final val CASSANDRAUSER             = "datastax-java-driver.basic.authentication.username"
  private final val CASSANDRAPASSWORD         = "datastax-java-driver.basic.authentication.password"

  private final val JDBCPROFILE  = "akka-persistence-jdbc.shared-databases.slick.profile"
  private final val JDBCURL      = "akka-persistence-jdbc.shared-databases.slick.db.url"
  private final val JDBCUSER     = "akka-persistence-jdbc.shared-databases.slick.db.user"
  private final val JDBCPASSWORD = "akka-persistence-jdbc.shared-databases.slick.db.password"
  private final val JDBCDRIVER   = "akka-persistence-jdbc.shared-databases.slick.db.driver"

  private final val RUNTIMEPERSISTENCE = "cloudflow.runtimes.akka.config.akka.persistence"

  private final val SECRETLOCATION    = "/etc/cloudflow-runner-secret/runtime-config.conf"
  private final val SECRETPERSISTENCE = "akka.persistence"

  private final val DBTYPE   = "dbtype"
  private final val HOST     = "dbhost"
  private final val PORT     = "dbport"
  ...
  // IN the case of local run persistence configuration is part of the configuration passed to streamlet
  def getPersistenceParamsLocal(config: Config, streamletName: String): Option[DBConfiguration] = {
  ...
  // In the case of running on cluster the easiest way to get configuration
  // is from secret
  def getPersistenceParamsCluster(): Option[DBConfiguration] = {

    val secretPath = Paths.get(SECRETLOCATION)

    if (Files.exists(secretPath)) {
      log.info(s"Loading persistence configuration from $secretPath")
      // secret takes precedence, since it contains config.
      val runtimeConfig = ConfigFactory.parseFile(secretPath.toFile)
      runtimeConfig.as[Option[Config]](SECRETPERSISTENCE) match {
        case Some(persistence) => // We have persistence
          internalPopulatePersistence(persistence)
        case _ => None
      }
    } else {
