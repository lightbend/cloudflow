 ...
 */
abstract class AkkaClusterStreamlet extends Streamlet {
  ...
  override protected final def createContext(config: Config): AkkaStreamletContext = {
    ...
    val clusterConfig: Config = if (!localMode) {
      ...
      akka.discovery.kubernetes-api.pod-namespace = ""
      akka.discovery.kubernetes-api.pod-label-selector = ""
    """).withFallback(config)
    } else {
      config
    }
    ...
    (for {
      streamletDefinition ← StreamletDefinition.read(clusterConfig)
    } yield {
      val updatedConfig = streamletDefinition.copy(config = streamletDefinition.config.withFallback(clusterConfig))
   ...
   */
  private[akkastream] def setContext(streamletContext: AkkaStreamletContext): AkkaClusterStreamlet = {
