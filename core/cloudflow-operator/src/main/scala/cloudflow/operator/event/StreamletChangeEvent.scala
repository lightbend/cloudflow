...
object ConfigChangeEvent {
  ...
  def detected[T <: ObjectResource](event: StreamletChangeEvent[T]) = s"Configuration change for streamlet ${event.streamletName} detected in application ${event.appId}."
}

object StatusChangeEvent {
  /** log message for when a StreamletChangeEvent is identified as a status change event */
  def detected[T <: ObjectResource](event: StreamletChangeEvent[T]) = s"Status change for streamlet ${event.streamletName} detected in application ${event.appId}."
}
...
object StreamletChangeEvent {
   ...
   */
  def fromWatchEvent[O <: ObjectResource](modifiedOnly: Boolean = false)(implicit system: ActorSystem): Flow[WatchEvent[O], StreamletChangeEvent[O], NotUsed] = {
    Flow[WatchEvent[O]]
  ...
  def toStatusUpdateAction[O <: ObjectResource](implicit system: ActorSystem): Flow[(Option[CloudflowApplication.CR], StreamletChangeEvent[O]), Action[ObjectResource], NotUsed] = {
            ...
            streamletChangeEvent match {
              case StreamletChangeEvent(appId, streamletName, _, watchEvent) ⇒
                watchEvent match {
                  ...
                  case _ ⇒ system.log.warning(s"Detected an unexpected change in $appId ${changeInfo(watchEvent)} in streamlet ${streamletName} (only expecting Pod changes): \n ${watchEvent}")
                }
            }
  ...
  def toConfigUpdateAction[O <: ObjectResource](implicit system: ActorSystem, ctx: DeploymentContext): Flow[(Option[CloudflowApplication.CR], StreamletChangeEvent[O]), Action[ObjectResource], NotUsed] = {
    Flow[(Option[CloudflowApplication.CR], StreamletChangeEvent[O])].map {
      case (Some(app), streamletChangeEvent) ⇒
        import streamletChangeEvent._
        app.spec.deployments.find(_.streamletName == streamletName).map { streamletDeployment ⇒
          ...
          val streamletChangeEventAction = EventActions.streamletChangeEvent(app.spec, streamletDeployment, namespace, watchEvent._object)
