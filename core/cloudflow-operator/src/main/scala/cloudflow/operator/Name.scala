...
object Name {
   ...
   * Some subsystem of Kubernetes cannot manage longer names.
   */
  private def truncateTo63CharactersWithSuffix(name: String, suffix: String): String = name.take(63 - suffix.length) + suffix
  ...
  private[operator] def makeDNS1039Compatible(name: String): String = {
    val normalized = Normalizer.normalize(name, Normalizer.Form.NFKD).toLowerCase.replaceAll("[_.]", "-").replaceAll("[^-a-z0-9]", "")
