...
abstract class RunnerActions[T <: ObjectResource](runner: Runner[T]) {
  protected def actions(
    ...
    val deleteActions = currentDeployments.filterNot(deployment ⇒ newDeploymentNames.contains(deployment.name))
      .flatMap { deployment ⇒
        Seq(
    ...
    val createActions = newDeployments.filterNot(deployment ⇒ currentDeploymentNames.contains(deployment.name))
      .flatMap { deployment ⇒
        Seq(
    ...
    val updateActions = newDeployments.filter(deployment ⇒ currentDeploymentNames.contains(deployment.name))
      .flatMap { deployment ⇒
