...
object BlueprintBuilder extends StreamletDescriptorBuilder {
  ...
  implicit class BlueprintOps(blueprint: Blueprint) {
    ...
    def define(streamletDescriptorsUpdated: Vector[StreamletDescriptor]): Blueprint =
      copy(streamletDescriptors = streamletDescriptorsUpdated).verify

    def upsertStreamletRef(
        streamletRef: String,
        className: Option[String] = None,
        metadata: Option[Config] = None): Blueprint =
      streamlets
        ...
        .getOrElse(className
          .map(streamletDescriptorRef =>
            use(StreamletRef(name = streamletRef, className = streamletDescriptorRef, metadata = metadata)))
          .getOrElse(blueprint))
      ...
      val newTopics = topics
        .map { topic =>
          topic
            .copy(
              producers =
                topic.producers.filterNot(port => VerifiedPortPath(port).exists(_.streamletRef == streamletRef)),
              consumers =
                topic.consumers.filterNot(port => VerifiedPortPath(port).exists(_.streamletRef == streamletRef)))
            .verify(verifiedStreamlets)
        }
        .filter(_.connections.nonEmpty)

      copy(streamlets = streamlets.filterNot(_.name == streamletRef), topics = newTopics).verify
    }
    ...
    def connect(topic: Topic, ports: Vector[String]): Blueprint = {
      ...
      val foundTopic = topics
        .find(_.id == topic.id)
        .map(t =>
          t.copy(producers = (t.producers ++ producers).distinct, consumers = (t.consumers ++ consumers).distinct))
