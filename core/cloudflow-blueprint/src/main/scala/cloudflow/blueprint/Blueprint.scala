...
final case class Blueprint(
  ...
  def upsertStreamletRef(
    ...
    streamlets
      ...
      .map { streamletRef ⇒
        ...
        copy(streamlets = streamlets.filterNot(_.name == streamletRef.name) :+ streamletRefWithMetadataUpdated)
          .verify
      }.getOrElse(
        className
  ...
  def use(streamletRef: StreamletRef): Blueprint =
    copy(streamlets = streamlets.filterNot(_.name == streamletRef.name) :+ streamletRef)
      .verify
  ...
  def connect(connection: StreamletConnection): Blueprint = {
    ...
    copy(connections = otherConnections :+ verifiedConnection)
      .verify
  }
  ...
  def disconnect(inletPortPath: String): Blueprint = {
    ...
    val toBeDeleted: StreamletConnection ⇒ Boolean =
      if (verifiedInlet.isRight) {
        con: StreamletConnection ⇒ VerifiedPortPath(con.to) == verifiedInlet.map(_.portPath)
      } else if (verifiedPortPath.isRight) {
        con: StreamletConnection ⇒ VerifiedPortPath(con.to) == verifiedPortPath
      } else {
        con: StreamletConnection ⇒ con.to == inletPortPath
      }
  ...
  private def verifyNoDuplicateStreamletNames(streamlets: Vector[StreamletRef]): Either[DuplicateStreamletNamesFound, Vector[StreamletRef]] = {
    val dups =
      streamlets
        .groupBy(_.name.trim())
        .flatMap {
  ...
  private def verifyUniqueInletConnections(
    ...
    val illegalConnectionProblems = verifiedStreamletConnections.groupBy(_.verifiedInlet).collect {
  ...
  private def verifyInletsConnected(
    ...
    val unconnectedPortProblems = verifiedStreamlets.flatMap { streamlet ⇒
      val unconnectedInlets = streamlet
        .descriptor
        .inlets
        .filterNot { inlet ⇒
  ...
  private def verifyVolumeMounts(streamletDescriptors: Vector[StreamletDescriptor]): Vector[BlueprintProblem] = {
      ...
      descriptor ⇒
        descriptor.volumeMounts.map { volumeMount ⇒
          ...
          val nonAbsolutePath = if (!new File(volumeMount.path).toPath.isAbsolute())
            Some(NonAbsoluteVolumeMountPath(descriptor.className, volumeMount.name, volumeMount.path)) else None

          Vector(invalidPath, emptyPath, nonAbsolutePath).flatten
        }
    }

    val invalidNames = streamletDescriptors.flatMap {
      descriptor ⇒
        descriptor.volumeMounts.map { volumeMount ⇒
    ...
    val duplicateNames = streamletDescriptors.flatMap {
      descriptor ⇒
    ...
    val duplicatePaths = streamletDescriptors.flatMap {
      descriptor ⇒
  ...
  private def verifyConfigParameters(streamletDescriptors: Vector[StreamletDescriptor]): Vector[BlueprintProblem] = {
    ...
    val duplicateConfigParametersKeysFound = {
      streamletDescriptors.flatMap { streamletDescriptor ⇒
        ...
        keys.diff(keys.distinct).distinct.map(duplicateKey ⇒
          DuplicateConfigParameterKeyFound(streamletDescriptor.className, duplicateKey)
        )
      }
    }
        ...
        descriptor ⇒
          descriptor.configParameters.map { configKey ⇒
            configKey.validationType match {
              ...
              case "duration" ⇒ configKey.defaultValue.fold[Option[BlueprintProblem]](None) {
              ...
              case "memorysize" ⇒ configKey.defaultValue.fold[Option[BlueprintProblem]](None) {
                memorySizeDefaultValue ⇒
