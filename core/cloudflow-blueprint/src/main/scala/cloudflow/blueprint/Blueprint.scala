      ...
      try {
        ...
        val topics = if (config.hasPath(TopicsSectionKey)) {
          getKeys(config, TopicsSectionKey).map { key ⇒
            val topic                    = getStringOrNone(config, s"$TopicsSectionKey.${key}.topic").getOrElse(key)
            val bootstrapServersOverride = getStringOrNone(config, s"$TopicsSectionKey.${key}.bootstrap.servers")
            val create                   = getBooleanOrNone(config, s"$TopicsSectionKey.${key}.create-at-deployment").getOrElse(true)
            val connections              = config.getStringList(s"$TopicsSectionKey.${key}.connections").asScala.toVector
            val kafkaConfig              = getConfigOrEmpty(config, s"$TopicsSectionKey.${key}")
