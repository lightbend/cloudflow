...
with existing applications which might use their proprietary encoding for messages.
To accomodate these use cases Cloudflow introduces `ExternalInlet` and `ExternalOutlet`,
which implement Kafka communications in `ByteArray` format, allowing user to explicitely
marshal/unmarshall data as port of the application code.
...
NOTE: `ExternalInlet` and `ExternalOutlet` are not intended for intra streamlets communications. They should only
