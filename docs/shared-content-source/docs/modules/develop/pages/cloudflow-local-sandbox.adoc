...
[source, HOCON]
----
cloudflow.streamlets.<streamlet-name>.<config-facet>.<config-key>=value
----

=== Supported Configuration Facets

In the sandbox runtime, only two configuration facets of streamlets are currently supported:

- configuration parameters, under the `config-parameters` facet.
- volume mounts, under the `volume-mounts` facet.

The general syntax for a configuration entry is:

[source, HOCON]
----
cloudflow.streamlets.valid-logger.<config-facet>.<config-key>=value
----
where the `config-facet` may be either `config-parameters` or `volume-mounts`.

We can provide values for these two facets as we explain in the following section.
...
=== Using Volume Mounts
In the Streamlet API, volume mounts --a Kubernets construct to access disk storage-- are declared in a similar way to configuration parameters, using a programmatic description of the volume mount that includes its desired mount path. 
...
In a Kubernetes deployment, that volume mount gets mapped to a permanent volume claim (PVC). 
