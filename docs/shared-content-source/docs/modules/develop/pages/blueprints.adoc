...
To create a working application, we must define how the `Streamlet`{empty}s connect together, using _topics_ as communication channels.
 by declaring which outlet from one `Streamlet` should connect with an inlet of another.

A blueprint specifies the streamlets used in an application and how they connect together.
In practical terms, a blueprint is a text file that uses the HOCON format to declare the streamlets in an application, the topics used as communication channels, and the connections between streamlets and topics.
 The blueprint must be defined in the file `src/main/blueprint/blueprint.conf` within the file structure of the Cloudflow application _sbt project_.
...
TIP: Using the configuration system, different streamlet instances of the same implementation can be given a different configuration so that they can work in customized ways.
...
- It becomes the name of the component in the application runtime (either on a cluster or on the local sandbox)
...
The streamlet references assigned in the `streamlets` section can be used in the `topics` section to connect streamlets and topics together, as we explore in the next section.
...
This model allows for an organic growth of an application over time, as newly implemented processes, modelled as streamlets, can be added to an existing application by connecting them to the list of consumers at the appropriate place in the data flow.
...
shared-config { //<3>                    
  ...
  producer-config = ${kafka-connection-config} {
    # more config can be added here from Kafka ProducerConfig (see org.apache.kafka.clients.producer.ProducerConfi)
  }
...
<2> Set the topic name explicitly, using the `topic.name` configuration key.
...
We can setup the Kafka provider configuration in a separate configuration file, like this:
...
The blueprint is automatically verified when application-level targets, like `buildApp` and `runLocal` are used.
