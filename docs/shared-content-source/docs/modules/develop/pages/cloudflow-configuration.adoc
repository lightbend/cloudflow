...
It also possible to add `volumes`, that contain secrets, to the pods. And then mount this volumes into the container with `volume-mounts`. Bear in mind that each `volume-mounts` name has to match a `volumes`  name, defined in the same config file. Here's a sample of how to set two `volumes` with two different secrets and how to mount them in a container.  
