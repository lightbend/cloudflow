...
It also possible to add `volumes` to pods where the source of the volume is a secret. Then these can be mounted into the container with `volume-mounts`. 

There is an exception for `volume-mounts` regards being abel to set and specific type of pod. Will cover that later on, now here below, a sample of how to configure two `volumes` with secrets and how to mount them.  
...
This configuration will mount `mysecret` into each `my-streamlet` pod on path `/mnt/folderA` as read only, while `mysecret2` will be mounted on path `/mnt/folderB` . Bear in mind that each `volume-mount` has to match a `volume` by name, defined in the same config.
...
Regards `volumes` and `volume-mounts` Spark runtime is the only runtime that allows to set different `volume-mounts` to each of its pods. Which are `driver` and `executor`. This will work as the following. The `volumes` are set in every pod but it's the `volume-mounts` that can be made specific to a type of pod. The sample below show how to:
