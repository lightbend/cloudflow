...
This configuration will mount `mysecret` into each `my-streamlet` pod on path `/mnt/folderA` as read only. While `mysecret2` will be mounted into `/mnt/folderB` . Bare in mind that each `volume-mount` has to point to a `volume` defined in the same config.
