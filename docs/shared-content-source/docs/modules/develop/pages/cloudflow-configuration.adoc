...
Apart from secrets `volumes` can also mount a Persistent Volumes Claim that already exist in the kubernetes cluster. They are also mounted into the container with `volume-mounts`. If the PVC doesn't not exist in the cluster will raise an error and stop the deployment or configuration.

Here's a sample, and further explanation, of how to configure two pvcs to be mounted on the containers that implements replicas of `my-streamlet`. 
...
This configuration will mount a preexisting Persistent Volume Claim `myclaim1` into each `my-streamlet` container on to the path `/mnt/folderA` as read-only, while `myclaim2` will be mounted on path `/mnt/folderB` as writable. As each `volume-mounts` name must match a `volumes` name, `volume-mounts.foo` matches `volumes.foo` and `volume-mounts.bar` matches `volumes.bar`.
...
| `volumes` | `kubernetes.pods.pod.volume.[name].pvc.readOnly = [true or false]`
...
Regarding `volumes` and `volume-mounts`, `runtime.spark` is the only runtime that allows to set different `volume-mounts` to each of its pods. More concretely, while the `volumes` have to be set as `cloudflow.runtime.spark.kubernetes.pods.pod.volumes` the `volume-mounts` can be set specifically as `cloudflow.runtime.spark.kubernetes.pods.driver.containers.container.volume-mounts` and/or the `cloudflow.runtime.spark.kubernetes.pods.executor.containers.container.volume-mounts`. This applies to both types of `volumes`, Secrets as well as Persistent Volume Claims.
  ...
  kubernetes.pods {
    ...
    executor {
      containers.container {
        volume-mounts {
          bar {
            mountPath = "/mnt/folderC/mysecret2"
            readOnly = true
          }
        }
      }
