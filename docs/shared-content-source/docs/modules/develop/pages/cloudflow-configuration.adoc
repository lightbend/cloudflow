...
and how to set labels to pod's first level section `metadata.labels`. 
...
Is important to clarify that flink configuration will not be able to set different labels to its pods. This is `task-manager` and `job-manager`. Only `cloudflow.runtimes.flink.kubernetes.pods.pod.labels` is allowed cloudflow.runtimes.flink.kubernetes.pods.[pod type].labels is not. Spark on the other hand can deal with different labeling for `driver` and `executor`.
