...
It is also possible to add `volumes`, that contain secrets, to the pods. And then mount these volumes into the container with `volume-mounts`. Bear in mind that each `volume-mounts` name has to match a `volumes`  name, defined in the same config file. Here's a sample of how to configure two `volumes` with two different secrets and how to mount them in the container.  The streamlet code can access the secret data in `mysecret1` in the file `/mnt/folderA/mysecret1`
  ...
  kubernetes.pods.pod {
   volumes {
    ...
    containers.container {
      volume-mounts {
        foo {
          mountPath = "/mnt/folderA"
          readOnly = true
        }
        bar {
          mountPath = "/mnt/folderB"
          readOnly = true
        }
      }
    }
...
This configuration will mount `mysecret1` into each `my-streamlet` container on to the path `/mnt/folderA/mysecret1` as read only, while `mysecret2` will be mounted on path `/mnt/folderB/mysecret2`. Inside these folders, you'll find as many files as values are under https://kubernetes.io/docs/concepts/configuration/secret/#decoding-a-secret[`data`] in the Kubernetes Secret mounted.
Each `volume-mounts` name must match a `volumes` name. In the example, `volume-mounts.foo` matches `volumes.foo` and `volume-mounts.bar` matches `volumes.bar`.
...
| `volume-mounts` | `pod.containers.container.volume-mounts.[name].mountPath = [some-path]`
| `volume-mounts` | `pod.containers.container.volume-mounts.[name].readOnly = [true or false]`
|===
...
Regarding `volumes` and `volume-mounts`, `runtime.spark` is the only runtime that allows to set different `volume-mounts` to each of its pods. More concretely, while the `volumes` have to be set under `cloudflow.runtime.spark.kubernetes.pods.pod` the 'volume-mounts' can be set specifically on the `cloudflow.runtime.spark.kubernetes.pods.driver` and/or the `cloudflow.runtime.spark.kubernetes.pods.executor`. The sample below shows how to:
  ...
  kubernetes.pods {
    ...
    driver {
      containers.container {
        volume-mounts {
          foo {
            mountPath = "/mnt/folderA/mysecret1"
            readOnly = true
          }
        }
      }
    ...
    executor {
      containers.container {
        volume-mounts {
          bar {
            mountPath = "/mnt/folderB/mysecret2"
            readOnly = true
          }
        }
      }
...
This configuration will mount `mysecret1` into any `driver` pod on path `/mnt/folderA/mysecret1` as read-only. While `mysecret2` will be mounted into any `executor` pod on path `/mnt/folderB/mysecret2`. Inside these folders, you'll find as many files as values are under https://kubernetes.io/docs/concepts/configuration/secret/#decoding-a-secret[`data`] in the Kubernetes Secret mounted.
