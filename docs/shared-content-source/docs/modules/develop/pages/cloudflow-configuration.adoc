...
This configuration will mount `mysecret` into each `my-streamlet` container on to the path `/mnt/folderA` as read only, while `mysecret2` will be mounted on path `/mnt/folderB`. Each `volume-mounts` name must match a `volumes` name. In the example, `volume-mounts.foo` matches `volumes.foo` and `volume-mounts.bar` matches `volumes.bar`.
...
This configuration will mount `mysecret` into any `driver` pod on path `/mnt/file1` as read only. While `mysecret2` will be mounted into any `executor` pod on path `/mnt/file2`.
