...
Regards `volumes` and `volume-mounts`, `runtime.spark` is the only runtime that allows to set different `volume-mounts` to each of its pods. More concretly, while the `volumes` have to be set under `cloudflow.runtime.spark.kubernetes.pods.pod` the 'volumen-mounts' can be set specifically on the `cloudflow.runtime.spark.kubernetes.pods.driver` and/or the `cloudflow.runtime.spark.kubernetes.pods.executor`. The sample below shows how to:
