...
The project structure of a Cloudflow application follows the general conventions and folder structure of a `sbt` project with a few additional considerations.
...
<3> `build.properties` file contains the `sbt` version 
...
There are two particular files that require your attention:
...
The Cloudflow plugin is the main enabler of any Cloudflow application development. 
Internally, it contains several `sbt` plugin definitions that provide functions like dependency management, Docker image setup, library definitions, and also the local _sandbox_, that lets you run the applications on the local machine.
...
WARNING: The version of the `sbt-cloudflow` plugin defines with version of Cloudflow you are using.
...
The `CloudflowLocalRunnerPlugin`:: The `CloudflowLocalRunnerPlugin` adds a the `runLocal` task to `sbt` that allows you to run complete Cloudflow application in your local development environment.
+
The `CloudflowLocalRunnerPlugin` does not require a explicit declaration in the build file. 
It is an _autoplugin_ that becomes enabled when the `CloudflowApplicationPlugin` is present.

=== Runtime-specific Plugins

Cloudflow provides out-of-the-box support for Akka, Apache Spark Structured Streaming, and Apache Flink in the form of specific implementations of the Streamlet API and the inclusion of supporting elements, like Docker base images and _runners_ that are able to launch each specific technology stack on Kubernetes.
...
Applications that use a single runtime are the most simple form of a Cloudflow application project structure and build definition.
...
Streamlets for each runtime must be into its own separate sub-project.

Next to that, we recommend to separate the datamodel into its own sub-project so that streamlet sub-projects can easily import the common data definitions.
