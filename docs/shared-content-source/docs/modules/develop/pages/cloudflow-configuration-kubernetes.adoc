...
The above example shows that `500mcpu` is requested, as well as `512Mi` of memory while being limited to use `4Gi` of memory.   
...
The above example shows how to set the environment variable `JAVA_OPTS=XX:MaxRAMPercentage=40.0` and `FOO=BAR` 
