...
If you enable plugin `CloudflowAkkaPlugin`, Cloudflow will add all of the Akka jars, necessary for a typical Akka streamlet implementation. But sometimes
you might need to add additional libraries. When doing so, it is necessary to make sure, that the versions
of the libraries that you are adding have the same version that the rest of of the Akka libraries.
The version used by Cloudflow are currently defined in the variable `cloudflow.sbt.CloudflowAkkaPlugin.AkkaVersion`.
So, if for example, you want to add Akka Persistent Query library, you can do it as shown below:
