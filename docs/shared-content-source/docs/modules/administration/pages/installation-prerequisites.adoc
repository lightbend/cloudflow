...
Before proceeding, make sure that `kubectl` for your Kubernetes distribution is correctly installed and access the Kubernetes cluster to install Cloudflow. Cloudflow supports a variety of distributions, see xref:versions.adoc[Supported component versions].
...
For production make sure to choose a suitable Kafka solution from an upstream provider.
