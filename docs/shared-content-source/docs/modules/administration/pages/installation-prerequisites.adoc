...
Before proceeding, make sure that `kubectl` for your Kubernetes distribution is correctly installed and access the Kubernetes cluster to install Cloudflow. Cloudflow supports a variety of distributions, see xref:versions.adoc[component versions].
