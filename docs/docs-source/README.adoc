...
In Cloudflow, we call `dev` the version in the making. That is, the _latest-greatest_ changes that aren't released yet.
The corresponding URL mapping is: https://cloudflow.io/docs/dev/index.html

Any other version, is addressed by its explicit version number. For example: https://cloudflow.io/docs/v1.3.2/index.html
...
| x.y.z-docs for the most recent x.y.z release | current | vx.y.z |
| x.y.z-docs for older releases | vx.y.z | vx.y.z |
|=== 


=== Adding a new version

Adding a version of the documentation require for a new release requires two steps:

. Create a branch for the release and make it the `current` version by updating version references.
. Update the previous release to demote it from `current` to its corresponding `vx.y.z` version.
...
We do that by creating a new `x.y.z-docs` branch and applying the following changes in the branch:

First, name the documentation modules with their corresponding version:
...
Then, change the internal version references to the new versions:
...
==== Demoting the previous release to 
...
In the branch that corresponds to the release, let's call it `x.y.z-docs`, make the following update:

* In `docs/docs-source/docs/antora.yml`, update `version: current` to `version: x.y.z`

Note that all internal versions should match this version already.
...
```
Then follow the advice in the comment :-)
The updated version would look like:

```
 branches: [master, vx.y.z-docs, v1.3.3-docs]  # versioned content - add branches here
```
