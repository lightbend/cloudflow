 ...
 * create a json file named `data.json` with the content 
 +
 .data.json
 [source, json]
 ----
 {"deviceId":"c75cb448-df0e-4692-8e06-0321b7703992","timestamp":1495545346279,"measurements":{"power":1.7,"rotorSpeed":23.4,"windSpeed":100.1}}
 ---
...
* When using this port-forward method, it's possible to use the same `send-local-data.sh` script that we used to send data to the local sandbox. Run the following command:
[source, bash]
----
$./send-local-data.sh 
----
