...
Update the repository list
  ...
  helm repo update
If you have not created a `cloudflow` namespace yet, first execute the following command:
  
    kubectl create ns cloudflow
Install the latest version of Strimzi.

  helm install strimzi/strimzi-kafka-operator --namespace cloudflow --generate-name --set watchNamespaces="{cloudflow}"
...
IMPORTANT: Make sure that the namespace 'cloudflow' exists before continuing. To create the `cloudflow` namespace, execute `kubectl create ns cloudflow`.
