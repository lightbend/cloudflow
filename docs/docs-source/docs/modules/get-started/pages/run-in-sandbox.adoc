...
[success] /.../sensor-data/src/main/blueprint/blueprint.conf verified.
       ...
       └──────┬─────┘       └──────┬─────┘         
         ...
         └─┬───────┘  │            │               
           ...
           v          │            v               
   ┌──────────────┐   │   ┌─────────────────┐      
   │[rotor-speeds]│   │   │[invalid-metrics]│      
   └──────┬───────┘   │   └────────────────┬┘      
          │           └────┐               │       
          │                │               │       
          v                v               v       
 ┌────────────────┐ ┌────────────┐ ┌──────────────┐
 │rotor-avg-logger│ │valid-logger│ │invalid-logger│
 └────────────────┘ └────────────┘ └──────────────┘
---------------------------- Streamlets per project ----------------------------
 sensor-data-scala - output file: file:/tmp/cloudflow-local-run3239069643364862589/sensor-data-scala-local.log
	...
	- mount [source-data-mount] available at [/tmp/cloudflow]
...
Now, we are ready to xref:setup-k8s-cluster.adoc[Set up a Kubernetes cluster].
