...
In the `streamlets` section of the blueprint, we declare the streamlets participating in the pipeline definition. 
...
Streamlet outlets are producers. Streamlet inlets are consumers. Consumers and producers of the same topic must have the same or compatible schemas.

Let's take this definition as an example:
[source,json]
----
include::{cloudflow-examples-version}@docsnippets:ROOT:example$sensor-data-scala/src/main/blueprint/blueprint.conf[topic-example]
----
