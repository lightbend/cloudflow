...
In this example we will use a Docker Hub (docker.io) registry.
...
ThisBuild / cloudflowDockerRepository := Some("<docker hub username>")
```

Publish the docker image of your project to the above registry. Make sure you are logged in with `docker login --username <docker hub username>` and then build and publish the application with `sbt buildApp`
...
If your Docker Hub is private, you need to supply the credentials to the Docker repository along with the CLI `deploy` command.
The following example reads a password from a file, and passes it to the `kubectl cloudflow deploy` command using STDIN:

```
$ cat my-password.txt | kubectl cloudflow deploy /path/to/sensor-data-scala/target/sensor-data-scala.json -u <docker hub username> --password-stdin
```

If your docker registry is public you can omit credentials with the `--no-registry-credentials` flag, which is shown in the following example:
