...
The Kafka broker bootstrap configuration string is a comma-separated list of host/port pairs to establish the connection to the Kafka cluster that Cloudflow will use. The configuration string should have the following format>
...
If you plan to write applications using Spark or Flink, the Kubernetes cluster will need to have a storage class of the `ReadWriteMany` type installed. This storage class name will be used when installing Cloudflow.
...
The `repo update` command is to make sure Helm has an up-to-date index of all Helm charts.
...
The `persistentStorageClass` parameter sets the storage class to use with Spark and Flink applications. This storage class, as previously mentioned, has to be of the type `ReadWriteMany`. In our example, we are using the `nfs` storage class, which we have installed in our cluster using the NFS Server Provisioner chart.
...
Installing the Cloudflow Helm chart.
...
Now we can install the Spark operator using Helm. Note that we use a specific version fo the Spark operator Helm chart.
...
After the Spark operator installation is completed, we need to adjust its webhook configuration.
