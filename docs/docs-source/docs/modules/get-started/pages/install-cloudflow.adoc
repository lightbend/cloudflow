...
The broker bootstrap configuration is a string with the address and port to each of the brokers in the Kafka cluster. It has the following format:

broker-1-address:broker-1-port, broker-2-address;broker-2-port

If you want to test Cloudflow we recommend using Strimzi, a third-party Kafka operator that can create and manage Kafka clusters. 
...
After the Spark operator is installed, we need to adjust its webhook configuration.
...
Once the environment variables are configured, we can execute the Helm command to install the Cloudflow enterprise components.
