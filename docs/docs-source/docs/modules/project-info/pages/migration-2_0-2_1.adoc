...
- Change your build, to use version 2.1.x of the sbt-cloudflow plugin, use the native plugin(s) from the cloudflow-contrib project, and rebuild your applications with `sbt buildApp`, as documented in https://lightbend.github.io/cloudflow-contrib/docs/0.1.1/get-started/index.html[Cloudflow contrib]
...
Note: For existing users of Cloudflow 2.0.25 it is important to note that Cloudflow must be upgraded with the helm charts, as described in the documentation. The Cloudflow kubectl plugin must also be upgraded to 2.1.x and existing applications must be rebuilt with the 2.1.x sbt-cloudflow plugin.
