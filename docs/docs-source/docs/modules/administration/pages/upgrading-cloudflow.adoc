...
NOTE: Migrating from Cloudflow 2.0.10, a new setting `kafkaClusters.default.bootstrapServers` is required, which replaces the `cloudflow_operator.bootstrapServers` setting, so be sure to set this to your existing kafka cluster in the helm upgrade command as shown below:
